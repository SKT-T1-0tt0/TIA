...
class VQGAN(pl.LightningModule):
    ...
    def forward(self, x, optimizer_idx=None, log_image=False):
        ...
        post_vq_conv = self.post_vq_conv(z)
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        #x = batch['stft']
        if optimizer_idx == 0:
            recon_loss, _, aeloss, perceptual_loss, gan_feat_loss = self.forward(x, optimizer_idx)
    ...
    def validation_step(self, batch, batch_idx):
        x = batch['video']  
        # x = batch['stft']
        recon_loss, _, perceptual_loss = self.forward(x)
