...
class VQGAN(pl.LightningModule):
    def __init__(self, args):
        ...
        self.content_encoder = Encoder(args.n_hiddens, args.downsample, args.image_channels, args.norm_type, args.padding_type)
        self.motion_encoder = Encoder(args.n_hiddens, args.downsample, args.image_channels, args.norm_type, args.padding_type)
        self.decoder = Decoder(args.n_hiddens, args.downsample, args.image_channels, args.norm_type)
        self.enc_out_ch = self.content_encoder.out_channels
    ...
    def forward(self, x, optimizer_idx=None, log_image=False):
        ...
        B, C, T, H, W = x.shape #torch.Size([8, 3, 16, 64, 64])
        c = self.content_encoder(x)
        m = self.motion_encoder(x)
        #z = c + m
        #z = torch.cat((c[:,:,0].unsqueeze(2),z[:,:,:-1]), 2) #torch.Size([8, 256, 16, 8, 8])
        z = self.pre_vq_conv(z)
        #c = self.pre_vq_conv(self.content_encoder(x))
        vq_output = self.codebook(z)
        post_vq_conv = self.post_vq_conv(vq_output['embeddings'])
        x_recon = self.decoder(post_vq_conv)
    ...
    def configure_optimizers(self):
        ...
        lr = self.args.lr
        opt_ae = torch.optim.Adam(list(self.content_encoder.parameters())+
...
class Decoder(nn.Module):
    def __init__(self, n_hiddens, upsample, image_channel=3, norm_type='group'):
        super().__init__()
...
class SamePadConv3d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, bias=True, padding_type='replicate'):
        ...
        pad_input = sum(pad_input, tuple()) #(1,1,1,1,1,1)
