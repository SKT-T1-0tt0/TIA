...
def collate_fn(batch):
    ...
    # # 如果 batch 为空，返回一个空的张量，或者选择适合的处理方式
    # if len(batch) == 0:
    #     # 例如返回一个空的张量，或者根据你的需要做其他处理
    #     return torch.tensor([])
...
class VideoData(pl.LightningDataModule):
    ...
    def _dataset(self, train):
        if hasattr(self.args, 'text_stft_cond') and self.args.text_stft_cond:
            ...
            Dataset = TIDataset
            dataset = Dataset(self.args.data_path, self.args.sequence_length, self.args.text_emb_model, train=train, resolution=self.args.resolution, image_channels=self.args.image_channels, text_len=self.args.text_seq_len, truncate_captions=self.args.truncate_captions, class_cond=self.args.class_cond)
    ...
    def add_data_specific_args(parent_parser):
        ...
        parser.add_argument('--text_emb_model', type=str, default='clip', choices=['bert', 'clip'])
        parser.add_argument('--audio_emb_model', type=str, default='STFT', choices=['STFT', 'wav2clip', 'audioclip', 'beats'])
...
class TAVDataset(data.Dataset):
    ...
    def __init__(self, data_folder, sequence_length, text_emb_model, audio_emb_model, 
        ...
        self.resolution = resolution
        self.sample_every_n_frames = sample_every_n_frames
        self.load_vid_len = load_vid_len
        self.text_emb_model = text_emb_model
        ...
        # self.video_paths = sum([glob.glob(osp.join(folder, 'mp4', f'*.mp4'), recursive=True)], [])
        # self.text_paths = sum([glob.glob(osp.join(folder, 'txt', f'*.txt'), recursive=True)], [])
        self.video_paths = [path.replace("/stft_pickle/", "/mp4/").replace(".pickle", ".mp4") for path in self.stft_paths]
        self.text_paths = [path.replace("/stft_pickle/", "/txt/").replace(".pickle", ".txt") for path in self.stft_paths]
        self.audio_paths = [path.replace("/stft_pickle/", "/wav/").replace(".pickle", ".wav") for path in self.stft_paths]
        # self.stft_paths = [path.replace("/mp4/", "/stft_pickle/").replace(".mp4", ".pickle") for path in self.video_paths]

        warnings.filterwarnings('ignore')
        cache_file = osp.join(folder, f"metadata_{self.load_vid_len}.pkl")
        if not osp.exists(cache_file):
            clips = VideoClips(self.video_paths, self.load_vid_len, num_workers=8)
            pickle.dump(clips.metadata, open(cache_file, 'wb'))
        else:
            metadata = pickle.load(open(cache_file, 'rb'))
            clips = VideoClips(self.video_paths, self.load_vid_len, _precomputed_metadata=metadata)
        ...
        if text_emb_model == 'bert':
            ...
            print('using clip pretrain model...')
            self.tokenizer = CLIPTokenizer.from_pretrained('tacm/modules/cache/clip-vit-large-patch14')
            self.transformer = CLIPTextModel.from_pretrained('tacm/modules/cache/clip-vit-large-patch14')
    ...
    def __getitem__(self, idx):
        ...
        if self.text_emb_model == 'bert':
            ...
            outputs = self.transformer(input_ids=tokens)[0]

            # 防止文本中存在多行（一般是回车造成）
            if outputs.shape[0] != 1:
                outputs = outputs[0:1]

            #tokenized_text = clip.tokenize(text, context_length=77, truncate=self.truncate_captions)
            #tokenized_text = self.clip_model.encode_text(tokenized_text)
        ...
        if self.audio_emb_model == 'STFT':
            ...
            return dict(**preprocess(video[start:end], self.resolution, sample_every_n_frames=self.sample_every_n_frames), 
                        text=outputs, raw_text=text, stft=stft, path=self.video_paths[video_idx])
        elif self.audio_emb_model == 'wav2clip' or self.audio_emb_model == 'audioclip' or self.audio_emb_model == "beats":
            audiofile = self.audio_paths[video_idx]
            if self.load_vid_len == 90:
                sr = 96000
            else:
                sr = 48000
