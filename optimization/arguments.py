...
def get_arguments(parent_parser):
    ...
    parser.add_argument(
        "--image_key", type=str, help="The key for getting image/video", default="video")
    parser.add_argument(
        "--cond_key", type=str, help="The key for getting condition", default="audio", choices=["audio", "stft"])

    # Diffusion
    parser.add_argument(
        "--diffusion_ckpt",
        type=str,
        default="saved_ckpts/ldm_opdif_0514/model420000.pt",
    )
    parser.add_argument(
        "--skip_timesteps",
        type=int,
    ...
    # Loss
    parser.add_argument(
        "--audio_guidance_lambda",
        type=float,
        help="Controls how much the video should look like the audio prompt",
        default=0,
    )
    parser.add_argument(
        "--text_guidance_lambda",
        type=float,
        help="Controls how much the 1st frame should look like the text prompt",
        default=1000,
    )
    parser.add_argument(
        "--direction_lambda",
        type=float,
        help="Controls the change direction distance between the image and the prompt",
        default=0,
    )
    parser.add_argument(
        "--range_lambda",
