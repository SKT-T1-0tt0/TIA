...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, context=None, mask=None, n_times_crossframe_attn_in_self=0,):
        ...
        #mask = np.zeros((q.shape[0], q.shape[1], k.shape[1]))
        #mask += np.tril(np.ones((seq_length, memory_length)))
...
class SpatialTransformer(nn.Module):
    ...
    def __init__(self, in_channels, n_heads, d_head,
        ...
        self.transformer_blocks = nn.ModuleList(
            [BasicTransformerBlock(inner_dim, n_heads, d_head, dropout=dropout, context_dim=context_dim, disable_self_attn=disable_self_attn,)
    ...
    def forward(self, x, context=None, n_times_crossframe_attn_in_self=0):
        ...
        for i, block in enumerate(self.transformer_blocks):
            ...
            x = block(x, context=context, n_times_crossframe_attn_in_self=n_times_crossframe_attn_in_self)
...
class TemporalTransformer(nn.Module):
    ...
    def __init__(self, in_channels, n_heads, d_head,
        ...
        self.transformer_blocks = nn.ModuleList(
            [BasicTemporalTransformerBlock(inner_dim, n_heads, d_head, dropout=dropout, context_dim=context_dim,
    ...
    def forward(self, x, context_temp=None):
        ...
        if not self.use_linear:
            x = rearrange(x, '(b f) c h w -> (b h w) c f', f=16).contiguous()
            x = self.proj_in(x)
            x = rearrange(x, '(bhw) c f -> (bhw) f c')
        if self.use_linear:
            x = rearrange(x, '(b f) c h w -> b (h w) f c', f=16).contiguous()
            x = self.proj_in(x)
