...
from .discriminator import NLayerDiscriminator, NLayerDiscriminator3D, hinge_d_loss
...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        if self.use_discriminator:
            self.image_discriminator = NLayerDiscriminator(3, 64, 3)
            self.video_discriminator = NLayerDiscriminator3D(3, 64, 3)
            self.disc_loss = hinge_d_loss
            self.image_gan_weight = 1
            self.video_gan_weight = 1
    ...
    def training_losses(self, model, x_start, t, c, c_temp, model_kwargs=None, noise=None):
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            if self.use_discriminator:
                ...
                x_start = rearrange(x_start, "(b f) c h w -> b c f h w", f=16)
                pred_xstart = rearrange(pred_xstart, "(b f) c h w -> b c f h w", f=16)
                B, C, T, H, W = pred_xstart.shape

                frame_idx = th.randint(0, T, [B])
                frame_idx_selected = frame_idx.reshape(-1, 1, 1, 1, 1).repeat(1, C, 1, H, W).to(x_start.device)
                # print(x_start.device, frame_idx_selected.device)
                frames = th.gather(x_start, 2, frame_idx_selected).squeeze(2)
                frames_recon = th.gather(pred_xstart, 2, frame_idx_selected).squeeze(2)

                self.image_discriminator.to(x_start.device)
                self.video_discriminator.to(x_start.device)
                logits_image_real, _ = self.image_discriminator(frames.detach())
                logits_video_real, _ = self.video_discriminator(x_start.detach())

                logits_image_fake, _ = self.image_discriminator(frames_recon.detach())
                logits_video_fake, _ = self.video_discriminator(pred_xstart.detach())
                ...
                terms["dloss"] = discloss

                opt_disc = th.optim.Adam(list(self.image_discriminator.parameters()) +
                                         list(self.video_discriminator.parameters()),
                                         lr=5e-5, betas=(0.5, 0.9))
                opt_disc.zero_grad()
                terms["dloss"].backward()
                opt_disc.step()
