...
# modified from https://github.com/Stability-AI/generative-models/blob/9d759324e914de6c96dbd1468b3a4a50243c6528/sgm/modules/diffusionmodules/openaimodel.py
import logging
import math
...
from .attention import SpatialTransformer, default, SpatialVideoTransformer, exists, AlphaBlender
...
class Upsample(nn.Module):
    ...
    def __init__(self, channels, use_conv, dims=2, out_channels=None, padding=1, third_up=False, kernel_size=3, scale_factor=2):
        super().__init__()
        ...
        if use_conv:
            self.conv = conv_nd(dims, self.channels, self.out_channels, kernel_size, padding=1)
    ...
    def forward(self, x):
        ...
        if self.dims == 3:
            ...
            x = F.interpolate(
                x, (t_factor * x.shape[2], x.shape[3] * self.scale_factor, x.shape[4] * self.scale_factor), mode="nearest"
            )
        else:
...
class VideoUNet(nn.Module):
    ...
    def forward(
        ...
        num_video_frames: Optional[int] = 16,
        image_only_indicator: Optional[th.Tensor] = None,
    ):
        assert (y is not None) == (
            self.num_classes is not None
