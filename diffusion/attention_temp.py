...
class BasicTransformerBlock(nn.Module):
    ...
    def forward(self, x, context_temp=None, n_times_crossframe_attn_in_self=0):
        return checkpoint(self._forward, (x, context_temp), self.parameters(), self.checkpoint)

    def _forward(self, x, context_temp=None, n_times_crossframe_attn_in_self=0):
        x = self.attn1(
            self.norm1(x),
            context=context_temp if self.disable_self_attn else None,
...
class TemporalTransformer(nn.Module):
    ...
    def __init__(self, in_channels, n_heads, d_head,
        ...
        self.ff_in = FeedForward(in_channels, dim_out=inner_dim, glu=True,)
        ...
        self.ff_out = FeedForward(inner_dim, dim_out=in_channels, glu=True,)
    ...
    def forward(self, x, context_temp=None):
        ...
        x = rearrange(x, '(b f) c h w -> (b h w) f c', f=16)
        x_in = x
        x = self.norm_in(x)
        x = self.ff_in(x)
        x = x_in + x
        ...
        for block in self.transformer_blocks:
            context_temp = repeat(context_temp, "b m d -> (b h w) m d", h=h, w=w)
            x = block(x, context_temp=context_temp, n_times_crossframe_attn_in_self=0)
