    ...
    with torch.no_grad():
        for i in tqdm(range(total_size // batch_size)):
            ...
            for bs in range(videos.shape[0]):
                ...
                frame0 = T.resize(videos_i[0], [model.visual.input_resolution, model.visual.input_resolution]).unsqueeze(0)
                frame0_embedding = model.encode_image(frame0)
                              
                for idx in range(1, videos_i.shape[0]):
                    image = T.resize(videos_i[idx], [model.visual.input_resolution, model.visual.input_resolution]).unsqueeze(0)
                    image_embed = model.encode_image(image)
                    dist = d_clip_loss(image_embed, frame0_embedding, use_cosine=True) 
