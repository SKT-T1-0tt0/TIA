import sys
sys.path.append('/_zhaominglustn/program/TAV')
import piq
import argparse
...
def calculate_audioclip_score(audio_files, video_files, resize, num_workers, print_256, sequence_length, idx, batch_size):
    ...
    device = "cuda" if torch.cuda.is_available() else "cpu"
    model, preprocess = clip.load("ViT-B/32", device=device)
    audioclip = AudioCLIP(pretrained=f'saved_ckpts/AudioCLIP-Full-Training.pt')
    with torch.no_grad():
        for i in tqdm(range(total_size // batch_size)):
            start = i * batch_size
            end = min(start + batch_size, total_size)
...
def main(args):
    ...
    for i, (audio_root, video_root) in tqdm(enumerate(zip(sorted(audio_folders), sorted(video_folders)))):
        ...
        if args.audio_emb_model == 'wav2clip':
            clip_i = calculate_wav2clip_score(audio_files, video_files, args.resize, args.num_workers, args.print_256, args.sequence_length, args.idx, args.batch_size)
        elif args.audio_emb_model == 'audioclip':
            clip_i = calculate_audioclip_score(audio_files, video_files, args.resize, args.num_workers, args.print_256, args.sequence_length, args.idx, args.batch_size)
