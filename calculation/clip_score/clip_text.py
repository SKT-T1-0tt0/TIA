...
def main(args):
    ...
    for i, (txt_root, video_root) in tqdm(enumerate(zip(sorted(txt_folders), sorted(video_folders)))):
        ...
        assert len(txt_files) == len(video_files)

        print(f"[{i}] Computing clip scores")
        clip_i = metrics_from_files(txt_files, video_files, args.resize, args.num_workers, args.print_256, args.idx, args.batch_size)
        if len(args.idx) == 0:
